...
- Do NOT add conditional type guards on generated types (e.g. `T extends object ? T : any`)
- Trust generated types as-is; never hedge with `any` as fallback
- Each generated `xxx.api.ts` must declare at the top: `const HOST = import.meta.env.VITE_API_HOST;`
- All URLs passed to fetchData must be prefixed with `${HOST}`
...
If two endpoints in the same segment produce the same function name,
expand the name by prepending the next left path segment (PascalCase) until unique.

Examples:
- `GET /user/list` → `getList`
- `GET /user/order/list`→ `getOrderList` (conflict resolved by prepending "order")
- `GET /user/order/detail`→ `getOrderDetail` (conflict resolved by prepending "order")
